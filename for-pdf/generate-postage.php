...
if (isset($_GET['auction_id']) && is_numeric($_GET['auction_id'])) {
    ...
    foreach ($winners as $winner) {
        ...
        $user_comment = $wpdb->get_var($wpdb->prepare(
            "SELECT user_comment FROM $table_postage_info WHERE user_id = %d AND auction_id = %d",
            $winner->ID,
            $auction_id
        ));
        ...
        $html .= '<div class="total-info">
                    <strong>Tuotteet yhteensä: </strong> <span class="items-total-price">' . esc_html($postageInfo['all_products_price']) . ' €</span><br>
                ...
                <div class="total-cost">
                    <strong>Yhteensä särkyvälisällä: <span class="grand-total">' . esc_html($postageInfo['total_price']) . ' €</span></strong>
                </div>';
    ...
    // Вывод комментария пользователя, если он есть
    if ($user_comment) {
        $html .= '<br><div class="user-comment"><strong>Kommentti:</strong> ' . nl2br(esc_html($user_comment)) . '</div>';
    }

    $html .= '</div>'; // Конец блока winner-item
