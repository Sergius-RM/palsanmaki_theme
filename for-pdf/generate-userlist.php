...
foreach ($current_auctions as $auction_id) {
    ...
    $current_auction_id = $auction_id->ID;
    ...
    foreach ($unique_product_ids as $product_id) {
        ...
        $html .= '<p class="auction_data"><span>pvm: ' . $date . '</span><span>paikka: ' . $location . '</span></p>
                    <p class="auction_data"><span>Hirvaskankaan Kauppahuone Oy, 2913179-2</span></p>';
        ...
        $products = $wpdb->get_results($wpdb->prepare("
            SELECT p.ID, p.post_title, pm2.meta_value as current_auction_id
            FROM {$wpdb->posts} p
            INNER JOIN {$wpdb->postmeta} pm ON p.ID = pm.post_id
            INNER JOIN {$wpdb->postmeta} pm2 ON p.ID = pm2.post_id AND pm2.meta_key = 'current_auction_id' AND pm2.meta_value = %d
            WHERE pm.meta_key = 'product_id' AND pm.meta_value = %s AND p.post_parent = %d
            ORDER BY CAST(SUBSTRING_INDEX(p.post_title, ' ', 1) AS UNSIGNED), p.post_title ASC
        ", $current_auction_id, $product_id, $auction_id));
        foreach ($products as $product) {
            $html .= '<tr class="product_item">
                        <td>' . esc_html($product->post_title) . '
                                <!---br><div style="font-size: 8px;">(auction ID: ' . esc_html($product->current_auction_id) . ')</div--->
                        </td>
